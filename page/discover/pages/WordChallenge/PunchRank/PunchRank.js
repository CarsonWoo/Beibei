// pages/WordChallenge/PunchLeaderboard/PunchLeaderboard.js
const app = getApp()
...
Page({
  ...
  data: {
    ...
    //本地图片路径
    rank_head: '/images/PunchRank.png',

    img_gold:'/images/gold.png',

    img_silver:'/images/silver.png',

    img_copper:'/images/copper.png',

    img_fish:'/images/right_fish.png',
  ...
  onLoad: function (options) {
    ...
    console.log(options.medallion_success)
    if (options.medallion_success==1){
      this.setData({

        isShowAcquireSuccess: true,
  ...
  loadData: function (token) {
    ...
    wx.request({
      url: app.globalData.HOST + "/various/show_word_challenge_rank.do",
      method: 'POST',
      header: {
      ...
      success: (res) => {
        ...
        if (data.status == 200) {
          ...
          //用户本人挑战打卡排行
          let punch_rank_Data = data.data
          let state = punch_rank_Data.clock
          let portrait = punch_rank_Data.portrait
          var user_rank = punch_rank_Data.user_rank
          let username = punch_rank_Data.username
          let word_number = punch_rank_Data.word_number
          let insist_day = punch_rank_Data.insist_day

          //其他用户挑战打卡排行
          var total_rank = punch_rank_Data.total_rank
          ...
          //     if (total_rank[i].word_number == total_rank[i - 1].word_number) {
          ...
          //     if (total_rank[i].word_number == punch_rank_Data.word_number) {
          ...
          //邀请排行榜第一的用户
          let top_invite_portrait = punch_rank_Data.top_invite_portrait
          let top_invite_reward = punch_rank_Data.top_invite_reward
          let top_invite_username = punch_rank_Data.top_invite_username

          var is_show_gold = true
          var is_show_silver = true
          var is_show_copper = true

          if (total_rank.length < 3) {
            is_show_copper = false
          }
          if (total_rank.length < 2) {
            is_show_silver = false
          }
          if (total_rank.length < 1) {
            is_show_copper = false
          }
          ...
          this.setData({
            ...
            show_gold: is_show_gold,
            show_silver: is_show_silver,
            show_copper: is_show_copper,

            top_invite_portrait: top_invite_portrait,
            top_invite_reward: top_invite_reward,
            top_invite_username: top_invite_username
