...
Page({
  ...
  onLoad: function (options) {
    //是需要看过激励视频
    if (getApp().globalData.isneedvideoAd == true){
      this.setData({
        isShowMask: true
      })
    }
    ...
    if (options.isFromWordList != undefined) {
      this.setData({
        isFromWordList: true,
        word_id: word_id
      })
    } else {
      var wrongCount = parseInt(options.wrongCount)
      var meaning_list = options.meaning.split("；")
      console.log(meaning_list)
      var list = []
      var j = 0
      if (meaning_list.length >= 1) {
      ...
      this.setData({
        ...
        word_id: word_id,
        word: options.word,
        word_symbol: options.symbol,
        paraphrase: options.paraphrase,
        basic_meaning: list.length > 1 ? list[0] : meaning_list,
        meaningList: list,
        pic: options.pic
      })
    }
  ...
  onvideoAdTap:function(){
    var HOST = app.globalData.HOST
    var token = app.globalData.token
    let videoAd = null
    if (wx.createRewardedVideoAd){
      videoAd = wx.createRewardedVideoAd({
        adUnitId: 'adunit-f7ea403d1c0f6bd5'
      })
    }
    console.log("广告对象："+videoAd)
    if (videoAd){
      videoAd.show()
        .catch(err => 
        {
      ...
      videoAd.onClose((status) => {
        if (status && status.isEnded || status === undefined) {
          console.log("激励视频观看完毕,下发奖励")
          getApp().globalData.isneedvideoAd = false
          that.setData({
            isShowMask: false
          })
          wx.request({
            url: HOST + '/home/uploadAdvertisingTime.do',
            method: 'POST',
            header: {
              'content-type': 'application/x-www-form-urlencoded',
              'token': token
        ...
        //记录激励视频点击次数
        wx.request({
          url: HOST + '/admin/addAdvertisementClickNum.do',
          method: 'GET',
          header: {
            'content-type': 'application/x-www-form-urlencoded',
          },
  ...
  onShareAppMessage: function () {
    ...
    var share_text = this.data.word + '  ' + this.data.meaning

    var share_img = this.data.pic
